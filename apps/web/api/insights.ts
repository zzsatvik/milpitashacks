...
import { generateAuditInsights, type LawnAnalysis } from "@terraview/shared";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!/^\d{5}$/.test(zipCode) || !req.body?.analysis) {
    return res.status(400).json({ error: "Expected { analysis, zipCode }" });
  }
